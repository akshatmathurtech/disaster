...
export default function RouteFinderWindow({
  ...
  const calculateRoutesForNodes = async (src: string, dst: string) => {
    ...
    try {
      ...
      const offlineRoutes = isDelhi ? generateOfflineDelhiRoutes() : generateOfflineSector4Routes();
      const areaNodes = isDelhi ? DELHI_FALLBACK_NODES : SECTOR4_FALLBACK_NODES;
      ...
      setRouteResponse({
        ...
        aiAnalysis: {
          overview: `Analyzed 5 candidate emergency routes in ${isDelhi ? 'Delhi NCR' : 'Sector 4'} from ${srcNode.name} to ${dstNode.name}.`,
          insights: offlineRoutes.map(r => ({
            ...
            preferableWhen: r.routeId === 1 ? `Ideal for time-critical emergency responders in ${isDelhi ? 'Delhi NCR' : 'Sector 4'}.` : `Recommended alternative for ${isDelhi ? 'Delhi NCR' : 'Sector 4'} transit.`,
          })),
  ...
  useEffect(() => {
    const areaNodesFallback = areaId === 'delhi-demo' ? DELHI_FALLBACK_NODES : SECTOR4_FALLBACK_NODES;
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '160px' }}>
            <label style={{ fontSize: '10px', fontWeight: 700, color: '#00e5ff', display: 'block', marginBottom: '3px' }}>
              ORIGIN (POINT A) — {areaId === 'delhi-demo' ? 'DELHI NCR' : 'SECTOR 4'}
            </label>
            <select
              value={sourceId}
          ...
          <div style={{ flex: 1, minWidth: '160px' }}>
            <label style={{ fontSize: '10px', fontWeight: 700, color: '#00e5ff', display: 'block', marginBottom: '3px' }}>
              DESTINATION (POINT B) — {areaId === 'delhi-demo' ? 'DELHI NCR' : 'SECTOR 4'}
            </label>
            <select
              value={destId}
      ...
      {isSameLocation ? (
        ...
        <>
          ...
          <div style={{ fontSize: '10px', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '4px', padding: '6px 10px', color: '#6ee7b7', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span>🤖 <b>AI DECISION SUPPORT ({areaId === 'delhi-demo' ? 'DELHI NCR' : 'SECTOR 4'})</b> — 5 analyzed routes. System will NOT auto-select. Operator choice required.</span>
